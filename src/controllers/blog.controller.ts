...
export const createBlog = async (req: Request, res: Response) => {
  try {
    const { title, slug, content, excerpt } = req.body as {
      title: string;
      slug?: string;
      content: string;
      excerpt?: string;
    ...
    for (const concept of existingConcepts) {
      const conceptVec = concept.embedding as number[] | null;
      if (!conceptVec || conceptVec.length !== embedding.length) continue;

      const score = cosineSimilarity(embedding, conceptVec);
      if (score >= THRESHOLD) matchedConceptIds.push(concept.id);
    }
    ...
    for (const oldBlog of otherBlogs) {
      const oldVec = oldBlog.embedding as number[] | null;
      if (!oldVec || oldVec.length !== embedding.length) continue;
